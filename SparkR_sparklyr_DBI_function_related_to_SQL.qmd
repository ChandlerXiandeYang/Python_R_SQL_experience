...
# Your environment and connection info
...
#=======================
# Stage 3 Equipment Data
#=======================
generate_equipment_data_Stage3 <- function() {
  ...
  LPDE_Signet_Etobicoke_Stage3 <- c(LPDE_Weston_Stage3_Equipment, LPDE_Signet_Stage3_Equipment, LPDE_Garyray_Stage3_Equipment, LPDE_Etobicoke_Stage3_Equipment)
  Sites_Stage3 <- c("LPDE_Weston_Stage3_Equipment", "LPDE_Signet_Stage3_Equipment", "LPDE_Garyray_Stage3_Equipment", "LPDE_Etobicoke_Stage3_Equipment")
  ...
  save(LPDE_Weston_Stage3_Equipment, LPDE_Signet_Stage3_Equipment, LPDE_Garyray_Stage3_Equipment, LPDE_Etobicoke_Stage3_Equipment,
       LPDE_Signet_Etobicoke_Stage3, Sites_Stage3,
       file = paste0(install_folder, "\\CV_SARWAS_LPDE\\LowPDE_Templates\\_CV_Low_PDE_List_of_Equipment3.RData"))
}

# Load equipment data on app start
generate_equipment_data_Stage3()
...
ui <- fluidPage(
  tags$head(
    tags$style(HTML("
      ...
      #stage3_generate_report, #stage3_download_report, #stage3_download_data, #stage3_generate_data {
      ...
      #stage3_generate_report:hover, #stage3_download_report:hover, #stage3_download_data:hover, #stage3_generate_data:hover {
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("stage3_site", "Select Site", choices = Sites_Stage3),
      uiOutput("stage3_equipment_dropdown"),
      dateInput("stage3_Time_Period_Initial", "Start Date", value = Sys.Date() - 10000),
      dateInput("stage3_Time_Period_End", "End Date", value = Sys.Date()),
      actionButton("stage3_generate_report", HTML("<b>Generate Equipment Report</b>")),
      verbatimTextOutput("stage3_progress_report"),
      downloadButton("stage3_download_report", "Download Equipment Report"),
      actionButton("stage3_generate_data", HTML("<b>Generate Equipment Excel Data</b>")),
      verbatimTextOutput("stage3_progress_data"),
      downloadButton("stage3_download_data", "Download Equipment Excel Data")
    ),
    ...
    mainPanel(
      ...
      textAreaInput("mstage3_aintenance_contact", "Contact Information", value = "xyang1@apotex.com, Tel:416-8888-888")
    )
  )
)
...
server <- function(input, output, session) {
  ...
  output$stage3_equipment_dropdown <- renderUI({
    req(input$stage3_site)
    equipment_list <- switch(input$stage3_site,
    ...
    selectInput("stage3_equipment", "Select Equipment", choices = equipment_list, multiple = FALSE)
  })
  
  Report_Writing_Identifier <- reactiveVal(NULL)
  
  observeEvent(input$stage3_equipment, {
    Report_Writing_Identifier(input$stage3_equipment)
  })
  
  observeEvent(input$stage3_generate_report, {
    req(input$stage3_equipment)
    equipment_names <- input$stage3_equipment
    Time_Period_Initial <- as.character(input$stage3_Time_Period_Initial)
    Time_Period_End <- as.character(input$stage3_Time_Period_End)
  ...
  output$stage3_download_report <- downloadHandler(
    filename = function() {
      paste0("0", input$stage3_equipment, ".pdf")
    },
    content = function(file) {
      pdf_file <- file.path(paste0(install_folder, "\\CV_SARWAS_LPDE\\Reports_and_Data_Stage3_LPDE\\", input$stage3_equipment, ".pdf"))
      file.copy(pdf_file, file)
    }
  )
  
  output$stage3_progress_report <- renderText({
    if (!isTruthy(input$stage3_generate_report)) {
      return("Progress: 0% Done")
    }
    "Progress: 100% Done"
  })
  
  current_queries <- reactive({
    req(Report_Writing_Identifier())
  ...
  observeEvent(input$stage3_generate_data, {
    req(input$stage3_equipment)
    Time_Period_Initial <- as.Date(input$stage3_Time_Period_Initial)
    Time_Period_End <- as.Date(input$stage3_Time_Period_End)
    ...
    withProgress(
      ...
      value = 0,
      {
        Report_Writing_Identifier <- input$stage3_equipment[1]
        Sys.setenv(REPORT_WRITING_IDENTIFIER = Report_Writing_Identifier)
        ...
        for (i in seq_along(current_queries_val)) {
          ...
          incProgress(1 / total_queries, detail = paste('Generated data for', sheet_name))
        }
        
        data_file_path <- paste0(install_folder, "\\CV_SARWAS_LPDE\\Reports_and_Data_Stage3_LPDE\\", input$stage3_equipment, ".xlsx")
        openxlsx::saveWorkbook(wb, file = data_file_path, overwrite = TRUE)
        setProgress(1, message = 'Done!')
      }
    )
  })
  
  output$stage3_download_data <- downloadHandler(
    filename = function() {
      paste0(input$stage3_equipment, ".xlsx")
    },
    content = function(file) {
      data_file <- paste0(install_folder, "\\CV_SARWAS_LPDE\\Reports_and_Data_Stage3_LPDE\\", input$stage3_equipment, ".xlsx")
      file.copy(data_file, file)
    }
  )
  
  output$stage3_progress_data <- renderText({
    if (!isTruthy(input$stage3_generate_data)) {
      return("Progress: 0% Done")
    }
    "Progress: 100% Done"
  })
...
#=======================
# Stage 2 Equipment Data
#=======================
generate_equipment_data_Stage2 <- function() {
  LPDE_Weston_Stage2_Equipment <- dbGetQuery(conn1, paste0("SELECT * FROM ", data_folder, ".cv_processed.LPDE_Weston_Stage2_Equipment"))[, 1]
  LPDE_Signet_Stage2_Equipment <- dbGetQuery(conn1, paste0("SELECT * FROM ", data_folder, ".cv_processed.LPDE_Signet_Stage2_Equipment"))[, 1]
  LPDE_Garyray_Stage2_Equipment <- dbGetQuery(conn1, paste0("SELECT * FROM ", data_folder, ".cv_processed.LPDE_Garyray_Stage2_Equipment"))[, 1]
  LPDE_Etobicoke_Stage2_Equipment <- dbGetQuery(conn1, paste0("SELECT * FROM ", data_folder, ".cv_processed.LPDE_Etobicoke_Stage2_Equipment"))[, 1]
  
  LPDE_Signet_Etobicoke_Stage2 <- c(LPDE_Weston_Stage2_Equipment, LPDE_Signet_Stage2_Equipment, LPDE_Garyray_Stage2_Equipment, LPDE_Etobicoke_Stage2_Equipment)
  Sites_Stage2 <- c("LPDE_Weston_Stage2_Equipment", "LPDE_Signet_Stage2_Equipment", "LPDE_Garyray_Stage2_Equipment", "LPDE_Etobicoke_Stage2_Equipment")
...
stage2_ui <- tabPanel(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("stage2_site", "Select Site", choices = Sites_Stage2),
      uiOutput("stage2_equipment_dropdown"),
      dateInput("stage2_Time_Period_Initial", "Start Date", value = Sys.Date() - 10000),
      dateInput("stage2_Time_Period_End", "End Date", value = Sys.Date()),
      actionButton("stage2_generate_report", HTML("<b>Generate Equipment Report</b>")),
      verbatimTextOutput("stage2_progress_report"),
      downloadButton("stage2_download_report", "Download Equipment Report"),
      actionButton("stage2_generate_data", HTML("<b>Generate Equipment Excel Data</b>")),
      verbatimTextOutput("stage2_progress_data"),
      downloadButton("stage2_download_data", "Download Equipment Excel Data")
    ),
    mainPanel(
      HTML("<p><b>To Generate And Download Annual Statistical Report:</b><br>
            1. Select a site.<br>
...
# Stage 2 Server
#=======================
stage2_server <- function(input, output, session) {
  output$stage2_equipment_dropdown <- renderUI({
    site_name <- input$stage2_site
    ...
    selectInput("stage2_equipment", "Select Equipment", choices = equipment_list, multiple = FALSE)
  })
  
  Report_Writing_Identifier <- reactiveVal(NULL)
  
  observe({
    req(input$stage2_equipment)
    Report_Writing_Identifier(input$stage2_equipment)
  })
  
  # --- Generate Report
  observeEvent(input$stage2_generate_report, {
    equipment_names <- input$stage2_equipment
    Time_Period_Initial <- as.character(input$stage2_Time_Period_Initial)
    Time_Period_End <- as.character(input$stage2_Time_Period_End)
    original_working_directory <- paste0(install_folder, "\\CV_SARWAS_LPDE\\LowPDE_Templates")
    withProgress(
      session = session,
      message = 'Generating Reports...',
  ...
  output$stage2_download_report <- downloadHandler(
    filename = function() {
      paste("0", input$stage2_equipment, ".pdf", sep = "")
    },
    content = function(file) {
  ...
  output$stage2_progress_report <- renderText({
    if (!isTruthy(input$stage2_generate_report)) {
      return("Progress: 0% Done")
    }
    "Progress: 100% Done"
  })
  ...
  # --- Generate Data
  observeEvent(input$stage2_generate_data, {
    Time_Period_Initial <- as.Date(input$stage2_Time_Period_Initial, format = "%Y-%m-%d")
    Time_Period_End <- as.Date(input$stage2_Time_Period_End, format = "%Y-%m-%d")
    withProgress(
      session = session,
      message = 'Generating Data...',
      detail = '',
      value = 0,
      {
        Report_Writing_Identifier <- input$stage2_equipment[1]
        Sys.setenv(REPORT_WRITING_IDENTIFIER = Report_Writing_Identifier)
  ...
  output$stage2_download_data <- downloadHandler(
    filename = function() {
      paste0(input$stage2_equipment[1], ".xlsx")
    },
    content = function(file) {
      xlsx_file <- file.path(paste0(install_folder, "\\CV_SARWAS_LPDE\\Reports_and_Data_Stage2_LPDE\\", input$equipment[1], ".xlsx"))
  ...
  output$stage2_progress_data <- renderText({
    if (!isTruthy(input$stage2_generate_data)) {
      return("Progress: 0% Done")
    }
    "Progress: 100% Done"
  })
...
#=======================
ui <- navbarPage(
  "Cleaning Validation - Low PDE App",
  stage2_ui
)

server <- function(input, output, session) {
  stage2_server(input, output, session)
}
...
stage2_ui <- tabPanel(
  "Stage 2",
  # --- Paste your Stage 2 UI code here ---
  # Make sure to rename input/output IDs with prefix 'stage2_'
...
stage2_server <- function(input, output, session) {
  # --- Paste your Stage 2 server code here ---
  # Keep quarto_render() and Excel download logic unchanged
  # Rename all input/output IDs to match 'stage2_' versions in the UI
  ...
  observe({
    ...
    if (input$stage2_generate_data > 0 && !is.null(current_queries_val)) {
      withProgress(
        session = session,
...
stage3_ui <- tabPanel(
  "Stage 3",
  # --- Paste your Stage 3 UI code here ---
  # Make sure to rename input/output IDs with prefix 'stage3_'
  tags$head(
    tags$style(HTML("
    .titlePanel { background-color: #337ab7; color: white; font-weight: bold; text-align: center; }
    #generate_report, #download_report, #download_data, #generate_data {
...
stage3_server <- function(input, output, session) {
  # --- Paste your Stage 3 server code here ---
  # Keep quarto_render() and Excel download logic unchanged
  # Rename all input/output IDs to match 'stage3_' versions in the UI
  
  output$stage3_equipment_dropdown <- renderUI({
    site_name <- input$stage3_site
    equipment_list <- switch(site_name,
                             "LPDE_Weston_Stage3_Equipment"=LPDE_Weston_Stage3_Equipment,
                             "LPDE_Signet_Stage3_Equipment"=LPDE_Signet_Stage3_Equipment,
                             "LPDE_Garyray_Stage3_Equipment"=LPDE_Garyray_Stage3_Equipment,
  ...
  output$stage3_download_report <- downloadHandler(
    filename = function() {
      paste("0", input$stage3_equipment, ".pdf", sep = "")
    },
    content = function(file) {
      pdf_files <- file.path(paste0(install_folder,"\\CV_SARWAS_LPDE\\Reports_and_Data_Stage3_LPDE\\", input$stage3_equipment, ".pdf"))
      pdftools::pdf_combine(pdf_files, output = file)
    }
  )
  ...
  observeEvent(input$stage3_generate_data, {
    Time_Period_Initial <- as.Date(input$stage3_Time_Period_Initial, format = "%Y-%m-%d")
    Time_Period_End <- as.Date(input$stage3_Time_Period_End, format = "%Y-%m-%d")
    ...
    withProgress(
      ...
      value = 0,
      {
        ...
        xlsx_file <- file.path(paste0(install_folder, "\\CV_SARWAS_LPDE\\Reports_and_Data_Stage3_LPDE\\", input$stage3_equipment[1], ".xlsx"))
  ...
  output$download_data <- downloadHandler(
    filename = function() {
      paste0(input$stage3_equipment[1], ".xlsx")
    },
    content = function(file) {
      xlsx_file <- file.path(paste0(install_folder, "\\CV_SARWAS_LPDE\\Reports_and_Data_Stage3_LPDE\\", input$stage3_equipment[1], ".xlsx"))
  ...
  observe({
    ...
    if (input$stage3_generate_data > 0 && !is.null(current_queries_val)) {
      withProgress(
        session = session,
        message = 'Generating Data for Tabs...',
        detail = '',
...
#========================
ui <- navbarPage(
  "Cleaning Validation App",
  stage2_ui,
  stage3_ui
)

server <- function(input, output, session) {
  callModule(stage2_server, "stage2")
  callModule(stage3_server, "stage3")
}
...
stage2_ui <- tabPanel(
  ...
  "))
    ...
    sidebarLayout(
      sidebarPanel(
        selectInput("site", "Select Site", choices = Sites_Stage2),
...
stage3_ui <- tabPanel(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("site", "Select Site", choices = Sites_Stage3),
      uiOutput("equipment_dropdown"),
      dateInput("Time_Period_Initial", "Start Date", value = Sys.Date() - 10000),
      dateInput("Time_Period_End", "End Date", value = Sys.Date()),
      actionButton("generate_report", HTML("<b>Generate Equipment Report</b>")),
